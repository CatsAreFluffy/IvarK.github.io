...
function openNotationOptions() {
	if (document.getElementById("mainnotationoptions1").style.display=="") {
		...
		document.getElementById("maxArguments").value=Math.min(player.options.psi.args,8)
...
function switchOption(notation,id) {
	if (notation=="scientific") {
		...
		if (id=="maxLength") {
			if (isNaN(value)||value<2||value>30) return
			player.options.psi.chars=value
		}
		if (id=="maxArguments") {
			if (isNaN(value)||value<1||value>8) return
		...
		if (id.startsWith("psiLetter")) {
			...
			player.options.psi.letter=letters[id.slice(9)]
		}
	}
	onNotationChange()
}
